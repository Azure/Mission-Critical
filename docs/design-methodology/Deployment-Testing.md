...
- ADO instances are hosted in a single Azure region that is chosen [at organization-level](http://docs.microsoft.com/azure/devops/organizations/accounts/change-organization-location).
...
- Within Azure regions that provide Availability Zone support, the [Premium ACR tier supports Zone Redundancy](http://docs.microsoft.com/azure/container-registry/zone-redundancy) to protect against zonal failure.

- [Tagged ACR images are mutable by default](http://docs.microsoft.com/azure/container-registry/container-registry-image-lock#scenarios), meaning that the same tag can be used on multiple images pushed to the registry.
  - In production scenarios, this may lead to unpredictable behavior that could impact application uptime.

- ACR supports [locking an image version or a repository](http://docs.microsoft.com/azure/container-registry/container-registry-image-lock) to prevent changes or deletes.
  ...
  - Locking Container Images doesn't protect against the ACR instance being deleted, but [Azure Resource Locks](http://docs.microsoft.com/azure/azure-resource-manager/management/lock-resources) can be used to achieve this.

- ACR in Premium tier also offers support to restrict a container registry to a given set of virtual networks and subnets through [Private Endpoints](http://docs.microsoft.com/azure/container-registry/container-registry-private-link).
...
- This method ensures that secrets do not need to be stored on the application platform but can be held in memory only.
  ...
  - A native Azure solution [Azure Key Vault referenced App Settings](http://docs.microsoft.com/azure/app-service/app-service-key-vault-references) is also available.
...
- The application itself needs to be able to authenticate to the secret management system. For AKS the latter can be achieved using [Pod-managed Identities](http://docs.microsoft.com/azure/aks/use-azure-ad-pod-identity), but that is currently (as of August 2021) still in preview.
...
- Apply a fully automated key-rotation process that runs periodically within the solution.
  - Use [key near expire notification](http://docs.microsoft.com/azure/key-vault/keys/how-to-configure-key-rotation#configure-key-near-expiry-notification) to get alerted on upcoming expiration.
...
Testing is a key concern for both the local developer experience ("[Inner Loop](http://docs.microsoft.com/dotnet/architecture/containerized-lifecycle/design-develop-containerized-apps/docker-apps-inner-loop-workflow)") and the complete DevOps lifecycle ("[Outer Loop](http://docs.microsoft.com/dotnet/architecture/containerized-lifecycle/docker-devops-workflow/docker-application-outer-loop-devops-workflow)"), which captures when developed code begins release pipeline processes on its journey to a production environment.
